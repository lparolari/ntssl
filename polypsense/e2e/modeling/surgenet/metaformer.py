# https://github.com/TimJaspers0801/SurgeNet/blob/562575f416d3ebed7ead3443cc538fbd79c3cd52/metaformer.py
...
class LayerNormGeneral(nn.Module):
    ...
    def forward(self, x):
        ...
        s = c2.mean(
            self.normalized_dim, keepdim=True
        )  # Ensure compatibility with TensorRT
...
class MetaFormer(nn.Module):
    ...
    def load_pretrained_weights(self, url):
        # expect the weight to be a local path
        pretrained_dict = torch.load(url, map_location="cpu")

        # remove classification head weights
        pretrained_dict = {
            k: v for k, v in pretrained_dict.items() if not k.startswith("head.")
        }
...
if __name__ == "__main__":
    # For CAFormer
    model = caformer_s18(
        num_classes=4, pretrained="SurgeNet", pretrained_weights=urls["SurgeNet"]
    )
    ...
    model = MetaFormerFPN(
        num_classes=4, pretrained="SurgeNet", pretrained_weights=urls["SurgeNet"]
    ).cuda()  # pretrained {ImageNet, SurgNet}
