    ...
    def forward(self, x, B, T, W, return_attn=False):
        ...
        if self.attention_type in ["space_only", "joint_space_time"]:
            ...
            if self.class_tokens != 1:
                ...
                aux_cls_token = rearrange(
                    aux_cls_token, "b t m -> (b t) m", b=B, t=T
                ).unsqueeze(1)
            else:
                aux_cls_token = None
            xs = xt
            ...
            if aux_cls_token is not None:
                ...
                aux_cls_token = torch.mean(
                    aux_cls_token, 1, True
            ...
            if aux_cls_token is None:
                ...
                x = torch.cat((init_cls_token, x, init_aux_cls_token), 1) + torch.cat(
                    (cls_token, res, aux_cls_token), 1
                )
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, get_all=False, get_attn=False):
        ...
        patch_token = rearrange(patch_token, "b (t n) m -> b t n m", b=B, t=T)
        patch_token = torch.mean(
            patch_token, 1
        )  # averaging predictions for every frame
...
class EndoFMLV(VisionTransformer):
    def forward(self, x):
        """
        Args:
        ...
        Returns:
            Tensor of shape [b, 1 + p, d], where p is the number of patch tokens
            and d is the embedding dimension.
        """
        # x has shape [b, s, c, h, w] but endofm requires [b, c, s, h, w], so we permute it
        x = x.permute(0, 2, 1, 3, 4)

        # get features (use_head=False is identical to call forward_features)
        cls, patch = super().forward(x, use_head=False) # [b, d], [b, p, d]

        # merge cls and patch tokens in one tensor
        cls = cls[:, None, :]  # [b, 1, d]
        return torch.cat([cls, patch], dim=1)  # [b, 1+p, d]

def get_endofmlv(ckpt_path=None):
    model = EndoFMLV(
        img_size=224,
        num_classes=0,
        patch_size=16,
    ...
    if ckpt_path:
        ...
        if len(missing_keys) > 0 or len(unexpected_keys) > 0:
            logging.warning(f"EndoFM-LV model loaded")
            logging.warning(f"  Missing keys: {missing_keys}")
            logging.warning(f"  Unexpected keys: {unexpected_keys}")
        else:
            logging.info("EndoFM-LV model loaded successfully")
